...
Before anything is committed, check that the repo will take every file, not just the folder: `git check-ignore -v .deliverable/brief.md .deliverable/assets/*`. Anything it prints means a rule covers it — say which and stop; never `git add -f` around a rule, that only hides the brief from the next clone. The folder alone is not enough: `git add .deliverable` on a folder that is not ignored still skips, silently, each file inside it that a rule like `*.sql` covers, and the loop then runs with a brief whose fixture never left this disk — a fresh agent reconstructs it from the brief's description, and a fixture written by the implementer satisfies the implementer's reading. Then the branch: if the current one is the default (`origin/HEAD`), propose one named after the goal and create it, and rewrite the stamp line's branch to match — the loop reads a brief stamped with another branch as stale. Otherwise the current branch is the one, and the stamp line already carries it. Other uncommitted changes in the tree are the user's and stay out of the commit — name them, because the loop refuses to start on a dirty tree.
...
Only `.deliverable/` goes in — `git add` with the path, never `-A`. Then assert the commit carries the folder whole: `diff <(git ls-files .deliverable) <(find .deliverable -type f | sort)` prints nothing, or a file was skipped and the push must not happen. Report the branch and the commit, and that the loop, wherever it runs, starts from that branch and hands off as one pushed commit on it, with the folder removed in that commit.

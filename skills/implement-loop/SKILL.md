...
SHOTS="$(git rev-parse --show-toplevel)/.implement-loop"; mkdir -p "$SHOTS"; grep -qx '.implement-loop' .git/info/exclude 2>/dev/null || echo '.implement-loop' >> .git/info/exclude   # the run's only artifact root — every capture writes here and nowhere else, and §8 deletes it whole. Inside the repo because the capture tools refuse any path outside a workspace root; excluded through .git/info/exclude rather than .gitignore so the exclusion itself never appears in the hand-off diff
if [ -s "$LOG" ]; then
...
The single ending, finished or not (§7). One diff, **uncommitted and unpushed**, in the working tree of `$START`. The hand-off message is the user's first look at a run they did not watch: open with the outcome — which definitions are met and which are not, each standing gap in the verifier's words — then where the diff is, the tag's name, and the calls that are theirs (§7). Write it for a reader with none of the run's vocabulary; the log holds the round-by-round detail.

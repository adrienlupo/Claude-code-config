---
name: implement-loop
description: Iterate on a settled brief until every definition of done is verifiably met — build, verify with fresh context, repeat. Depth is sized by an entry scan, not chosen; every agent's model and effort come from the ventilate skill. The deliverable is one commit on the starting branch, pushed, with the brief removed in it so it never reaches a PR. Use when the user asks to run the implement loop, has a brief from the deliverable skill, or says "implement the brief".
...
- The brief and its assets are **read-only input**. Do not edit them, do not let a builder or the sweep clean them away. A definition rewritten mid-run has not been met, it has been deleted. `deliverable` committed the folder on the starting branch and pushed it, so a cloud agent with nothing but the checkout finds it; it leaves the tree at hand-off (§8) and nowhere earlier — the hand-off commit carries its removal, so the brief never lands in a PR. `## Out of scope` is a wall, not a preference: outside the builder's closed prompt (§4 step 1) and the wip branch (§3), it is the only thing bounding the run's blast radius.
...
```js
const LIMITS = {   // which of these bind: the counts, because the loop holds them. `budget` binds only when a `+Nk` directive set the runtime's `budget.total` — and then the runtime throws at the total instead of stopping cleanly, which is why step 7 stops at budget minus reserve before it can; with no directive nothing meters and `reserve` is bookkeeping. No per-call timeout exists
...
SHOTS="$(git rev-parse --show-toplevel)/.implement-loop"; mkdir -p "$SHOTS"; grep -qx '.implement-loop' .git/info/exclude 2>/dev/null || echo '.implement-loop' >> .git/info/exclude   # the run's only artifact root — every capture writes here and nowhere else, and §8 deletes it whole. Inside the repo because the capture tools refuse any path outside a workspace root; excluded through .git/info/exclude rather than .gitignore so the exclusion itself never appears in the hand-off commit
if [ -s "$LOG" ]; then
  ...
  [ -z "$(git status --porcelain -- ':/' ':!/.deliverable')" ] || { echo 'dirty tree — stop and ask the user'; exit 1; }   # the brief folder is the one thing allowed to be untracked: `deliverable` commits it, but a brief the user kept local is input, not their unsaved work
...
**Preconditions this file cannot enforce.** Its two hardest constraints are prompt text: the builder's sandbox (§4 step 1) and a judge's no-repo rule (invariant 3). Nothing here catches a subagent that ignores either — a markdown skill holds no permission over what an agent does. So running this unattended is safe only where the host's own permission configuration already denies what the sandbox forbids: force-push, destructive shell, installs and network, writes outside the repo root. A plain push is the one exception the host cannot deny for the whole run, because the hand-off (§8) makes exactly one — so for builders it stays a prompt rule, and the log's paths-touched lines are what would show a builder breaking it. Where it cannot, that is a reason not to start, not something to prompt harder about. Concurrency is the same shape: branch and log are shared state, and the resume path above cannot tell a crashed run from a live one, so one loop per repo is a precondition to confirm before taking either — not a check this loop can perform. The repo's `.gitignore` is the third: every round commits with `git add -A` (§4 step 6), and the standing setup is exactly what produces build output, caches and local env files — anything it writes into the tree that the ignore file does not already cover is committed and lands in the hand-off commit, so ignore those paths and keep credentials outside the repo root before starting.
...
1. `build` the failing definitions; every attempt after the first climbs one rung of the build ladder (`ventilate`), and a gap surviving the top rung is a real plateau where one surviving only the cheap rungs is not a finding. Require back the paths it changed and the definition each serves — a report never asked for is never written. **The builder's prompt is the closed list below and nothing else — no brief, no round log, no verdict history beyond the one line in the first item — so every constraint on a builder lives here:**
   ...
   - it may write tests, never weaken them: relaxing an assertion, deleting a failing case, or editing or removing the brief folder or a named suite's scenarios (invariant 8) is the loop's cheapest path to green, and it is the deletion §1 forbids aimed at the instrument instead of the definition.
...
You orchestrate. You never build. Your context is the only thing that persists across every round and the only place the whole picture lives; keep it for judgment — verdicts, the log's facts, the calls that end a run — and out of material, which belongs to agents.
...
The exit is the one a finished run takes: gate the current state if the reserve allows, then run **all of §8**, every step of it. An unfinished run leaves no more behind than a finished one. Then report the standing gap, the commit so far, and the calls that are the user's — widen the scope, adjust the definition, extend the budget, take it as it stands. None of these is a failure. A run that ends silently is indistinguishable from one that failed.

## 8. Hand-off

The single ending, finished or not (§7). **One commit on `$START`, pushed** — the run's whole tree, squashed, with the brief folder removed in it. The hand-off message is the user's first look at a run they did not watch: open with the outcome — which definitions are met and which are not, each standing gap in the verifier's words — then the commit and the branch it went to, the tag's name, that the brief was removed and is readable from the tag, and the calls that are theirs (§7). Write it for a reader with none of the run's vocabulary; the log holds the round-by-round detail. The commit message is that same message in git's shape, written to `$LOG.msg` before the script runs: subject, the brief's `## Goal` in one line; body, one line per definition — met, or its gap — then the tag's name. A reader of `git log` on a branch a cloud agent pushed has nothing else to go on.
...
[ -s "$LOG.msg" ] || { echo "no commit message at $LOG.msg — write it first"; exit 1; }
...
# 3. verify BEFORE committing, while the index holds exactly the tag's tree — must print nothing
git diff --cached --stat --exit-code "$RUN" || { echo "tree does not match $RUN — stop"; exit 1; }
if git diff --cached --quiet "$SHA"; then
  git reset -q --hard "$SHA"; echo "nothing built — $START untouched, brief still in place"   # no commit and no push: the branch stays exactly as `deliverable` left it, so the next attempt has a brief to read
else
  # 3a. the brief was input, never output: out of the index and the tree before the commit, so the branch `deliverable` pushed it to loses it in the same commit that carries the work, and a PR from that branch — brief added on it, removed on it — never shows it. Only the default location: a brief read from a path the user gave is their file and stays
  git rm -r -q --ignore-unmatch .deliverable; rm -rf .deliverable
  # 3b. one commit, squashed — the wip branch's round commits are the run's bookkeeping, not the user's history; the tag keeps them. Then the run's only push, plain: a rejection means the remote moved under the run, and the answer is fetch, rebase this one commit, push again — never --force, a teammate's commits on the branch are not the run's to overwrite
  git commit -q -F "$LOG.msg" && { git push -u origin "$START" || echo "PUSH REJECTED — the commit is on $START locally; rebase onto the remote and push, and say so in the hand-off"; }
fi
# 3c. prove the ventilation (ventilate §5): its histogram over this session's subagent transcripts, appended to "$LOG" as `ventilation: …` — a fable count above zero is a defect the hand-off names, with the transcript ids
# 4. only now — the sentinel dies with the branch it names, the record does not
git branch -D implement-loop-wip && mv "$LOG" "$LOG.$(date +%Y%m%d-%H%M%S).done" && rm -f "$LOG.progress" "$LOG.msg"
...
Step 2 refuses on a dirty tree, which `read-tree` would overwrite without a backup, and on a moved `$START` (§3). Step 3 runs before anything is committed: if it prints, stop and report — the work is safe in the tag, the branch and log still exist to re-enter on, and improvising a repair is how it gets lost. Step 3b is the one push in the run, and it is the hand-off's, not a builder's: on a cloud agent the container dies with the session, and a commit that never reached the remote died with it — which is also why an unfinished run (§7) pushes the same way, with its gaps in the message, rather than leaving partial work where nobody can fetch it. Step 4 clears the sentinel and the artifacts: a log left at `$LOG` sends the next run on this repo to a branch that no longer exists — renamed, not deleted, because the round history is what the human reads afterwards. Tear the standing setup (§4) down with it, and `$SHOTS` with both — screenshots and response bodies are a round's evidence, not its record, and a run that leaves a directory of them beside the repo has left the user something to identify and delete by hand months later. The tag holds what the commit does not: the round-by-round commits, and the brief — `git show "$RUN":.deliverable/brief.md` is where `retro` reads it once the tree no longer has it. It is local and stays local; name it in the hand-off, say it is the user's to delete once the branch is merged or discarded, and never delete it yourself. The commit is all the run leaves on the branch — no PR: opening one, or slicing the commit into a stack for review, is the `stack-this` skill's job; never start it yourself.

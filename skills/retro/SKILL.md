...
- `.deliverable/brief.md` if `implement-loop` ran — the brief the loop was aimed at. The loop removes it from the tree at hand-off, so when it is gone read it from the run's tag: `git show "$(sed -n 's/^tag: //p' "$(git rev-parse --git-dir)"/implement-loop.log.*.done | tail -1)":.deliverable/brief.md`. When it is still there, read it now: the next `deliverable` run offers to discard it. Find the session that wrote it too, when there is one: the brief's stamp line has a date, and the transcript in `$DIR` from that day containing `deliverable.md` writes is the deliverable session. That is where "was the skill followed" gets answered — without it the reviewer can judge the brief but not the skill that produced it.
- The loop's run log: `$(git rev-parse --git-dir)/implement-loop.log.<stamp>.done` — §8 renames it rather than deleting it, so a finished run's round lines are there, one per definition per round, with the gap in the verifier's words. A bare `.log` at that path is a run in flight or one that died; say which.
